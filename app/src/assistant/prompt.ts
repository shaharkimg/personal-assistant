...
export function buildSystemPrompt(ctx: PromptContext): string {
  ...
  lines.push(
    ...
    "- Email (Gmail): to go through mail, searchEmail 'in:inbox newer_than:2d -category:promotions -category:social', read what looks important, then give a short summary: what needs a reply, what's waiting on others, deadlines. Offer tasks / waiting-for items / reply drafts; create them only if the user agrees. Replies go through sendEmail (confirmation card).",
    "- Meetings with other people: find addresses (findEmailAddress, then searchContacts), check the time (findAvailableTimes when vague), then scheduleMeeting — it sends a real invitation and adds it to the calendar after the user confirms.",
